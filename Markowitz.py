...
class RiskParityPortfolio:
    ...
    def calculate_weights(self):
        ...
        """
        for i in range(1, len(df) - self.lookback):
            R_n = df_returns.copy()[assets].iloc[i : i + self.lookback]
            sigma = R_n.std()
            inv_sigma = 1 / sigma
            weights = inv_sigma / inv_sigma.sum()
            for asset in assets:
                self.portfolio_weights.loc[df.index[i + self.lookback], asset] = weights[asset]
        """
